 ...
 *
 * QXmppOutgoingClientPrivate::connectToHost(const QString &host, quint16 port) 做了简单更改
 */
...
void QXmppOutgoingClientPrivate::connectToHost(const QString &host, quint16 port)
{
    ...
    QSslConfiguration cfg = this->config.sslconfig();
    // override CA certificates if requested
    if (!this->config.caCertificates().empty()) {
        cfg.setCaCertificates(config.caCertificates());
    }
    q->socket()->setSslConfiguration(cfg);
    // respect proxy
...
void QXmppOutgoingClient::connectToHost()
{
    ...
    if (!d->config.host().isEmpty() && d->config.port()) {
        d->connectToHost(d->config.host(), static_cast<quint16>(d->config.port()));
        return;
    }
...
void QXmppOutgoingClient::_q_dnsLookupFinished()
{
    if (d->dns.error() == QDnsLookup::NoError &&
        ...
        d->connectToHost(d->config.domain(), static_cast<quint16>(d->config.port()));
    }
}
