...
UDPWRAPPERSHARED_EXPORT int CreateUdpSocketList(uint16_t group, const char *host, uint16_t port, int socket_num)
{
    ...
    for(int i=0; i<socket_num; i++){
        ...
        tv_out.tv_sec = 2;
        tv_out.tv_usec = 0;
        setsockopt(static_cast<EB_SOCKET>(fd), SOL_SOCKET, SO_RCVTIMEO, &tv_out, sizeof(tv_out));
#else
        int tv_out=2000;
        setsockopt(static_cast<EB_SOCKET>(fd), SOL_SOCKET, SO_RCVTIMEO,
...
UDPWRAPPERSHARED_EXPORT void SendDataOut(int sks_index,  uint8_t* const data, uint16_t pic_id, int width, int height, int sample, int u_sec)
{
    ...
    int toLen = static_cast<int>(sizeof(sockaddr_in));
        ...
        while (size - index > 512) {
           ...
           // w_status = send(sks->socket_fd, reinterpret_cast<const char*>(dst), static_cast<int>(p_size), 0);
            ...
            if(sks==head && u_sec > 0) std::this_thread::sleep_for(std::chrono::milliseconds(1));
        }
...
UDPWRAPPERSHARED_EXPORT void StartListener(uint16_t port, M_CALLBACK callback){
    ...
    while (*CTN){
          ...
          m_ssize_t rl = recvfrom(static_cast<SOCKET>(udp), reinterpret_cast<char*>(dst),
           ...
           //printf("pack_info %d, %d  %d\r\n", pk.pic_id, pk.pic_index, pk.head_flag);
           if(pk.head_flag == 123){
...
#else
                   sendto(udp, reinterpret_cast<const char*>(dst),
                          static_cast<int>(p_size), 0, &client_addr, sk_len);
#endif
               }else{
